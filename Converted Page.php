 ...
 * Minnesota Gopher 5 (MN3) — Pick 5 from 01–47, no bonus ball
 * Joomla 5.x + PHP 8.1+
 *
...
/**
 * Game config — Minnesota Gopher 5
 * Pick 5 from 01–47, no bonus ball
 */
$gameId        = 'MN3';
$gId           = $gameId;
$mainMin       = 1;
...
if (isset($stateName, $gName) && (string) $stateName !== '' && (string) $gName !== '') {
    $doc->setTitle('Results Intelligence — ' . $stateName . ' ' . $gName . ' Numbers Analysis | LottoExpert.net');
    $doc->setMetaData('description', 'Explore ' . $stateName . ' ' . $gName . ' lottery draw history. View hot and cold numbers, frequency charts, recency analysis, and draw context. Powered by LottoExpert SKAI intelligence.');
}
...
/**
 * --------------------------------------------------------------------------
 * Window comparison (50 vs 300 draws)
 * --------------------------------------------------------------------------
 */
...
/**
 * --------------------------------------------------------------------------
 * Draw history summary for current draw (main balls only — no bonus ball)
 * --------------------------------------------------------------------------
 */
$drawHistoryRows = [];
...
/**
 * --------------------------------------------------------------------------
 * Structured data (JSON-LD) — WebPage + Dataset
 * --------------------------------------------------------------------------
 */
$sSname = isset($stateName) ? (string) $stateName : '';
$sGname = isset($gName)     ? (string) $gName     : '';
$sGameLabel = ($sSname !== '' && $sGname !== '') ? ($sSname . ' ' . $sGname) : 'Lottery';

$jsonLdWebPage = [
    '@context'    => 'https://schema.org',
    '@type'       => 'WebPage',
    'name'        => $sGameLabel . ' Results Intelligence | LottoExpert.net',
    'description' => 'Historical frequency, recency, hot and cold number analysis, and draw context for ' . $sGameLabel . ' lottery. Powered by LottoExpert SKAI intelligence.',
    'url'         => $canonicalNoQuery,
    'inLanguage'  => 'en-US',
    'isPartOf'    => ['@type' => 'WebSite', 'name' => 'LottoExpert.net', 'url' => 'https://lottoexpert.net'],
    'about'       => ['@type' => 'Thing', 'name' => $sGameLabel],
    'keywords'    => $sGameLabel . ' lottery results, ' . $sGameLabel . ' frequency analysis, hot cold numbers, draw history, recency, LottoExpert',
];

$jsonLdDataset = [
    '@context'         => 'https://schema.org',
    '@type'            => 'Dataset',
    'name'             => $sGameLabel . ' Draw Frequency Analysis Dataset',
    'description'      => 'Analysis dataset for ' . $sGameLabel . ' Pick 5 drawings (numbers 01–47). Includes frequency counts, recency in draws, hot and cold number identification, and window-based distribution insights across selectable draw windows.',
    'creator'          => ['@type' => 'Organization', 'name' => 'LottoExpert.net', 'url' => 'https://lottoexpert.net'],
    'license'          => 'https://lottoexpert.net',
    'variableMeasured' => [
        ['@type' => 'PropertyValue', 'name' => 'Draw Frequency',   'description' => 'Number of times each main ball was drawn in the selected analysis window'],
        ['@type' => 'PropertyValue', 'name' => 'Recency (Draws)',   'description' => 'Number of draws elapsed since each ball last appeared'],
        ['@type' => 'PropertyValue', 'name' => 'Active Numbers',    'description' => 'Numbers with the highest draw counts in the current window'],
        ['@type' => 'PropertyValue', 'name' => 'Quiet Numbers',     'description' => 'Numbers with the longest absence from recent draws'],
    ],
];
?>
<script type="application/ld+json">
<?php echo json_encode($jsonLdWebPage, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT); ?>
</script>
<script type="application/ld+json">
<?php echo json_encode($jsonLdDataset, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT); ?>
</script>
<style>
:root{
  --skai-blue:#1C66FF;
...
.skai-chart-frame--tall{height:900px}
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-logo" aria-hidden="<?php echo $logo['exists'] ? 'false' : 'true'; ?>">
          <?php if ($logo['exists'] && $logo['url'] !== '') : ?>
            <img
              ...
              alt="<?php echo htmlspecialchars((string) $stateName . ' ' . (string) $gName . ' lottery logo', ENT_QUOTES, 'UTF-8'); ?>"
              width="110"
              height="110"
              loading="lazy"
              decoding="async"
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            <span class="skai-ball skai-ball--main" aria-label="Ball <?php echo htmlspecialchars(lePad2($p1), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($p1), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main" aria-label="Ball <?php echo htmlspecialchars(lePad2($p2), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($p2), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main" aria-label="Ball <?php echo htmlspecialchars(lePad2($p3), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($p3), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main" aria-label="Ball <?php echo htmlspecialchars(lePad2($p4), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($p4), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main" aria-label="Ball <?php echo htmlspecialchars(lePad2($p5), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($p5), ENT_QUOTES, 'UTF-8'); ?></span>
          </div>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history. Each chart below summarizes the key behavioral signals in your current analysis window.
        </p>
      </div>
    </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current draw easier to interpret. It shows how recently each latest number was previously seen and how the perspective shifts when you compare a short recent window with a broader historical one. Use this as context, not as prediction.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          ...
          <div class="skai-card-body">
              ...
              <div class="skai-history-list">
                <?php foreach ($drawHistoryRows as $row) : ?>
                  <div class="skai-history-item">
                    <div class="skai-history-name">Ball <?php echo htmlspecialchars((string) $row['label'], ENT_QUOTES, 'UTF-8'); ?></div>
            ...
            <?php else : ?>
              <p style="color:var(--text-soft);font-size:14px;">No draw data available.</p>
            <?php endif; ?>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;<?php echo (int) $mainMax; ?>, drawn as a horizontal bar chart for easy scanning. The right panel shows recency distance for every number — how many draws have passed since each last appeared.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--ember">
            ...
            <span class="skai-card-sub">Draws since last appearance for each main number (01&ndash;<?php echo (int) $mainMax; ?>)</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame skai-chart-frame--tall">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. A taller bar in the recency chart means the number has been absent for more draws.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use this table for exact counts, recency labels, and personal number tracking. Quick filters help narrow the view. The tracking panel below the table keeps your selected numbers visible while you move between sections.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        ...
        <div class="skai-note" style="margin:0 16px 16px;">
          Tracking is local to this page view. It is intended as a lightweight comparison aid while you move between the overview, tables, and advanced SKAI tools. Frequency context: each number in this game can appear in any of the 5 ball positions drawn from the range 01&ndash;<?php echo (int) $mainMax; ?>.
        </div>
      </div>
    </div>
  </section>
  ...
  <section id="tools" class="skai-section" aria-labelledby="tools-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          The results page establishes context. These tools take that context into deeper modeling and structured exploration. Use them after reviewing frequency and recency to extend your analysis.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-tool-grid">
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Use when you want a model-driven complement to the historical view shown on this page. Provides a structured AI-generated perspective on draw patterns.
            </p>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. <?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?> draws <?php echo (int) $mainBallCount; ?> main numbers from a pool of <?php echo (int) $mainMax; ?> (01&ndash;<?php echo (int) $mainMax; ?>). Each draw is independent. The purpose of this page is to make the recent behavior of the game easier to understand, compare, and carry into deeper SKAI analysis. Active numbers are identified by count within the selected window; quiet numbers are identified by recency distance within the same window. Neither category is predictive on its own.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function renderCharts() {
    ...
    if (recencyCanvas && chartData.mainLabels && chartData.mainLabels.length) {
      chartInstances['recencyChart'] = new Chart(recencyCanvas.getContext('2d'), {
        ...
        options: (function () {
          var opts = commonBarOptions(true);
          opts.plugins.tooltip = {
            enabled: true,
            callbacks: {
              label: function (ctx) {
                return ctx.parsed.x + ' draws ago';
              }
            }
          };
          return opts;
  ...
  function bindTrackers() {
    ...
    bindGroup(
      ...
      mainWrap,
      'skai-chip--main',
      'Select numbers above to create a short tracked set for comparison across this page.'
    );
